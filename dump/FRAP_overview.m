...
%%
idx = [9 1]%peakIdxLMB(1,:);
[num2str(idx) ' : ' FRAPdirs{idx(1)} ' / ' oibfiles{idx(1)}{idx(2)}]
...
%     end
    ...
    % bn = Bcorr (=Braw) because otherwise we would enforce Braw = 0
    ...
    %allOut{i}.RratCorr = allOut{i}.Araw./(1 - allOut{i}.ArawCyt); 
    % R after nuclear bleach, corrected for inadvertent bleach
    %RnbTilde = @(kin, kout, cs, ns, bn, bc) Rnb(kin, kout, cs, ns)*...
    %(bn + (bc-bn)*An(kin, kout, cs, ns))/(bc - (bc-bn)*Ac(kin, kout, cs, ns));
...
% X = (allOut{i}.bn + allOut{i}.Araw)./(allOut{i}.bc - allOut{i}.ArawCyt);
% Y = allOut{i}.Rrat;

X = allOut{i}.ArawCyt./allOut{i}.Araw;
Y = allOut{i}.ncrinit;
a = X./Y;
Yp = mean(a)*Y;
...
for i = 9:11%[3 7]%[1 3 8]%9:10%1:8
    ...
    set(gca,'FontSize', fs)
    set(gca,'FontWeight', 'bold')
    saveas(gcf,fullfile(dataDir, ['measuredValsCyt_' names{i} '.png']));
    %close;
end
...
for i = 1:3
    ...
    input{i}.rp = mean(instruct{i}.ncrfin); %uncorrected
    %input{i}.rp = mean(instruct{i}.ncrinit.*instruct{i}.(Aname)./(1-instruct{i}.(Acname))); %corrected
    
    input{i}.rlmb = mean(instructp{i}.ncrinit);
    ...
    input{i}.sigrlmb = std(instructp{i}.ncrinit)/sqrt(sum(instructp{i}.Ncells)-1);
...
alpha = 1;
%output = fitKineticModelNoLMBFixAlpha(input, alpha);
...
for i = 1:3
    ...
    %fprintf(['Rnb:\t' num2str([Rnbfit input{i}.rp Rnbfit/input{i}.rp],N) '\n']);
    fprintf(['Rnb:\t' num2str([Rpfit*output{i}.alpha input{i}.rp],N) '\n']);
...
%input = manualInput;
%output = fitKineticModelNewFitAlpha(input);
output = fitKineticModelFitAlphaBeta(input);
%output = fitKineticModelFitAlphaReduced(input);
...
for i = 1:3
    ...
    disp('--------------------------');
    fprintf(['An:\t' num2str([An(output{i}.kin, output{i}.kout, output{i}.cs, output{i}.ns) input{i}.A],N) '\n']);
    fprintf(['Ac:\t' num2str([output{i}.beta*Ac(output{i}.kin, output{i}.kout, output{i}.cs, output{i}.ns) input{i}.Ac],N) '\n']);
    fprintf(['k:\t' num2str([k(output{i}.kin, output{i}.kout) input{i}.k],N) '\n']);
    fprintf(['Almb:\t' num2str([An(output{i}.kin, output{i}.koutp, output{i}.cs, output{i}.ns) input{i}.Almb],N) '\n']);
    fprintf(['Aclmb:\t' num2str([output{i}.beta*Ac(output{i}.kin, output{i}.koutp, output{i}.cs, output{i}.ns) input{i}.Aclmb],N) '\n']);
    ...
    Rrat = Rpfit/Rfit;
    fprintf(['r:\t' num2str([Rfit*output{i}.alpha input{i}.r],N)  '\n']);
    fprintf(['rlmb:\t' num2str([RfitLMB*output{i}.alpha input{i}.rlmb],N) '(' num2str(input{i}.sigrlmb,N) ')\n']);
...
%%
% predicted R after nuclear bleach,
% has to be corrected for individual measurements, but involves fit output
    
RnbBC = @(kin,kout,bn,bc,RnbTilde)...
    RnbTilde./(1+ bn./bc - RnbTilde.*(bn./bc)*kap(kin,kout)/(1-kap(kin,kout)));


i = 3;
j = i;
RnbBCval = RnbBC(output{i}.kin, output{i}.kout, allOut{j}.bn, allOut{j}.bc, allOut{j}.ncrfin);
%[RnbBCval allOut{j}.ncrfin]
%mean([RnbBCval allOut{j}.ncrfin]./allOut{j}.ncrinit)
mean([RnbBCval allOut{j}.ncrfin])/mean(allOut{j}.ncrinit)
...
for i = 1:3
    ...
    disp(['Rp: ' num2str([R(output{i}.kin, output{i}.koutp, output{i}.cs, output{i}.ns) input{i}.rlmb],N)]);
    disp(['kp: ' num2str([k(output{i}.kin, output{i}.koutp) input{i}.klmb],N)]);
end
