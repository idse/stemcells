...
f1 = @(p) [ (k(p(1),p(2))   - input{1}.k)/input{1}.sigk,...
            (k(p(1),p(3))   - input{1}.kp)/input{1}.sigkp,...
            (k(p(1),p(4))   - input{2}.k)/input{2}.sigk,...
            (k(p(1),p(5))   - input{2}.kp)/input{2}.sigkp,... 
            (k(p(1),p(6))   - input{3}.k)/input{3}.sigk,...
            (k(p(1),p(7))   - input{3}.kp)/input{3}.sigkp ];

% initial values
kin0 = 0.001;
kout0 = 0.003;
koutp0 = kout0/10;
p1init = [kin0 kout0 koutp0 kout0 koutp0 kout0 koutp0];

% actual fitting
options.Algorithm = 'trust-region-reflective';
options.TolFun = 1e-10;
options.Display = 'off';
[pfit,~,~,~,~,~,jacobian] = lsqnonlin(f1, p1init, lb1, ub1, options);

for i = 1:3
    output{i}.kin = pfit(1);
    output{i}.kout = pfit(2*(i-1)+2);
    output{i}.koutp = pfit(2*(i-1)+3);
end
...
kin = output{1}.kin;
koutu = output{1}.kout;
koutpu = output{1}.koutp;
koutpk = output{2}.kout;
koutppk = output{2}.koutp;
kouta = output{3}.kout;
koutpa = output{3}.koutp;

f2 = @(p) [  (A(kin,koutu,p(1),p(2))         - input{1}.A)/input{1}.sigA,...
            (R(kin,koutu,p(1),p(2),p(7))   - input{1}.R)/input{1}.sigR,...
            (A(kin,koutpu,p(1),p(2))         - input{1}.Ap)/input{1}.sigAp,...
            (R(kin,koutpu,p(1),p(2),p(7))   - input{1}.Rp)/input{1}.sigRp,...
            (A(kin,koutpk,p(3),p(4))         - input{2}.A)/input{2}.sigA,...
            (R(kin,koutpk,p(3),p(4),p(7))   - input{2}.R)/input{2}.sigR,...
            (A(kin,koutppk,p(3),p(4))        - input{2}.Ap)/input{2}.sigAp,...
            (R(kin,koutppk,p(3),p(4),p(7))  - input{2}.Rp)/input{2}.sigRp,...
            (A(kin,kouta,p(5),p(6))     - input{3}.A)/input{3}.sigA,...
            (R(kin,kouta,p(5),p(6),p(7)) - input{3}.R)/input{3}.sigR,...
            (A(kin,koutpa,p(5),p(6))     - input{3}.Ap)/input{3}.sigAp,...
            (R(kin,koutpa,p(5),p(6),p(7))- input{3}.Rp)/input{3}.sigRp ];
...
ns0 = 0.5;
alpha0 = 1;
p2init = [cs0 ns0 cs0 ns0 cs0 ns0 alpha0];

% actual fitting
options.Algorithm = 'trust-region-reflective';
options.TolFun = 1e-10;
options.Display = 'off';
[pfit,~,~,~,~,~,jacobian] = lsqnonlin(f2, p2init, lb, ub, options);

for i = 1:3
    output{i}.cs = pfit(2*(i-1)+1);
    output{i}.ns = pfit(4*(i-1)+2);
