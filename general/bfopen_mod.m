function [result] = bfopen_mod(id, varargin)
% Modified version of bfopen created by AW 5/23/16. Allows for reading only
% limited number of series. series numbers are now 6th argument to function
% useful for reading only the primary image for vsi (run with seriesNum =
% 1)
% 
% Idse 7/21/2016 -> added plane argument to read single channel
%
% Open microscopy images using Bio-Formats.
%
% SYNOPSIS r = bfopen(id)
%          r = bfopen(id, x, y, w, h, seriesNum)
%          r = bfopen(id, x, y, w, h, seriesNum, planeIdx)
%
% Input
...
%
%    seriesNum - an array contain the number of the series to read
%
%    planeIdx - plane from the series to read
%
% Output
%
...
if nargin > 5 && max(varargin{5}) <= numSeries
    seriesToUse = varargin{5};
else
    seriesToUse = 1:numSeries;
end
...
for s = seriesToUse
    ...
    for i = planeIdx
        ...
        if any(cellfun(@isempty, varargin))
            ...
            if nargin > 5
                arr = bfGetPlane(r, i, varargin{1:4});
            else
