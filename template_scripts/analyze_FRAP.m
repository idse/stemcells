...
meta = Metadata(fullfile(dataDir,oibfile));
...
%     ratioMean(bad) = NaN;
    ...
    plot(t, cumsum(ratioMean - mean(baseline)),'LineWidth',2,'Color',colors(wellidx,:))
    %ylim([0.3, 2]);
...
plot([t(1) t(end)],[1 1]*mean(baseline),'--k','LineWidth',2);
hold off
