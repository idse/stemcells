...
'clearBorder',true, 'minAreaStd', 1, 'minSolidity',0.95, 'minArea',3000);
...
for fi = 2%:numel(oibfiles)
    ...
    for i = 1:nTracks
        ...
        if FrappedCytoIdx(i)    
            ...
            tracenorm = (S4cytTrace - min(S4cytTrace))/(max(S4cytTrace) - min(S4cytTrace));
            S4tracesCombinedCytNorm = cat(1, S4tracesCombinedCytNorm, tracenorm);

            %nuclear
            S4tracesCombinedNuc = cat(1, S4tracesCombinedNuc, S4nucTrace);
            tracenorm = (S4nucTrace - min(S4nucTrace))/(max(S4nucTrace) - min(S4nucTrace));
...
%%
    ...
    %%
    plot(results.tracesNucNorm')
    func = @(p1,p2,p3,x) p1*exp(-x*p2) + p3;
    x = 1:size(results.tracesNucNorm,2);
    hold on
    plot(func(0.88,0.015,0,x));
    hold off
